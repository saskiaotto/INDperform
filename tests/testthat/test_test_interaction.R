...
# model = test$model[[2]]
# ind_vec = init_tbl$ind_train[[1]]
# press_vec = init_tbl$press_train[[5]]
# t_var_vec = init_tbl$press_train[[3]]
# name_t_var = "Pwin"
# k = 4; a = 0.2; b = 0.8
...
test_that("test warnings and errors", {
  ...
  # no model sign.
  expect_error(test_interaction(init_tbl,
  	 mod_tbl[1:2, c("id", "ind", "press", "model_type", "p_val", "excl_outlier", "model")],
  	 select_interaction(mod_tbl)[c(1,5), ]))
})
