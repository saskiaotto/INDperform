...
plot_acf <- function(x_var, y_var) {
  p <- ggplot2::ggplot(data = NULL, ggplot2::aes_(x = x_var,
    ...
    ggplot2::geom_abline(intercept = 0, slope = 0) +
    ggplot2::geom_abline(intercept = 0.4, slope = 0, lty = 2,
      colour = "red") +
  	 ggplot2::geom_abline(intercept = -0.4, slope = 0, lty = 2,
    colour = "red") +
  	 ggplot2::labs(x = "Lag", y = "ACF") +
    ggplot2::lims(y = c(-1, 1)) +
    ggplot2::scale_x_continuous(breaks = seq(0, max(x_var), 2),
      labels = seq(0, max(x_var), 2)) +
    plot_outline()
  return(p)
}
...
plot_resid <- function(model_fitted, model_resid) {
  p <- ggplot2::ggplot(data = NULL,
  	   ggplot2::aes_(x = model_fitted, y = model_resid)) +
    ggplot2::geom_point() +
    ggplot2::geom_abline(intercept = 0, slope = 0) +
...
plot_qq <- function(model_resid, theo_quan) {
  ...
  p <- ggplot2::ggplot(data = NULL,
  	 ggplot2::aes_(x = theo_quan, y = sort(model_resid,
  	 	 na.last = TRUE))) +
    ggplot2::geom_point() +
...
plot_gcvv <- function(x_var, y_var, lab, best_t_val) {
  if (!is.null(x_var)) {
    p <- ggplot2::ggplot(data = NULL,
    	ggplot2::aes_(x = x_var, y = y_var)) +
     ggplot2::geom_line() +
     ggplot2::geom_vline(xintercept = best_t_val, colour = "red") +
     ggplot2::labs(x = all.vars(lab$formula)[3], y = "GCVV") +
    plot_outline()
  } else {
    p <- plot_empty()
  }
...
plot_all_diag <- function(p1, p2, p3, p4, p5, p6, title) {
  cowplot::ggdraw() +
		cowplot::draw_plot(p1, x = 0, y = 0.5, width = 0.3,
    height = 0.4) +
		cowplot::draw_plot(p2, x = 0.3, y = 0.5,
    width = 0.3, height = 0.4) +
  cowplot::draw_plot(p3, x = 0.6, y = 0.5, width = 0.3,
      height = 0.4) +
		cowplot::draw_plot(p4, x = 0, y = 0.1, width = 0.3,
    height = 0.4) +
  cowplot::draw_plot(p5, x = 0.3, y = 0.1, width = 0.3,
    height = 0.4) +
		cowplot::draw_plot(p6, x = 0.6, y = 0.1, width = 0.3,
    height = 0.4) +
...
#' @rdname plot_outline
# Function to plot an empty plot --> also used in plot_model()
plot_empty <- function() {
	 p <- ggplot2::ggplot() +
	 	plot_outline() +
	 	ggplot2::labs(x = "", y = "") +
