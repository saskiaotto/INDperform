...
select_model <- function(gam_tbl, gamm_tbl){
		...
		gam_tbl <- check_input_tbl(
				...
				var_to_check = c("id", "ind", "press", "aic"),
				dt_to_check = c("integer", "character", "character", "numeric")
		)
		gamm_tbl <- check_input_tbl(
				gamm_tbl, tbl_name = "gamm_tbl", parent_func = "model_gamm()",
				var_to_check = c("id", "ind", "press", "aic"),
				dt_to_check = c("integer", "character", "character", "numeric")
		)
	 ...
	 # helper function
		get_best_model <- function(models) {
				...
				pos_min <- which(min_aic == suppressWarnings(min(min_aic, na.rm = TRUE)))
				best_mod <- models$corrstruc[pos_min]
				if(best_mod == Inf) best_mod <- "none"
				return(best_mod)
		}
		...
		for(i in gamm_tbl$id) {
				final_tab$corrstruc[final_tab$id == i] <- corrstruc[which(names(corrstruc) == as.character(i))]
		}

		#merge gam(m)_tbl to final_tab
		suppressMessages(final_tab1 <- dplyr::left_join(final_tab[final_tab$corrstruc == "none", ],
				gam_tbl))
		suppressMessages(final_tab2 <- dplyr::left_join(final_tab[!final_tab$corrstruc == "none", ],
				gamm_tbl))
