...
plot_thresh <- function(thresh_sublist, choose_thresh_gam ) {
  ...
  select <- purrr::map_dbl(thresh_sublist,
  	 ~suppressWarnings(as.numeric(try(.$mgcv, silent = TRUE))))
 ...
 # below/above threshold value
  ...
  pred_above <- add_pred(mod = model, t_val = t_val,
    which = "above")
	 ...
	 # Find limits
	 y_lim <- range(c(below_t_val[ ,1], above_t_val[ ,1], # all observed values
		  pred_above$ci_up, pred_above$ci_low,                  # all upper CI
				pred_below$ci_up, pred_below$ci_low), na.rm = TRUE)   #all lower CI

	 # Plot model
  plot_x <- function(model, data, pred, title, col, y_lim) {
    x_lab <- names(data)[2]
    y_lab <- names(data)[1]
    ...
    p <- ggplot2::ggplot() +
    	 ggplot2::geom_polygon(data = NULL,
	    	  ggplot2::aes_(x = poly_x, y = poly_y),
	    	  fill = col, alpha = 0.2) +
	    	ggplot2::geom_point(data = NULL,
	    		 ggplot2::aes_(x = data$press, y = data$ind), colour = col,
	    		 size = 1) +
	    	ggplot2::geom_line(ggplot2::aes_(x = pred$press_seq,
	    		 y = pred$pred), colour = col) +
	    	ggplot2::labs(x = x_lab, y = y_lab) +
	    	ggplot2::expand_limits(x = range(model$model[,2]),
	    		 y = y_lim) +
  ...
  title1 <- bquote(paste(.(all.vars(model$formula)[3]) <= .(round(t_val, digits = 2)),
			 " (p = ", .(round(summary(model)$s.pv[1],3)), ")"))
	 title2 <- paste0(all.vars(model$formula)[3], " > ",
	 	 round(t_val, digits = 2), " (p = ",
			 round(summary(model)$s.pv[2],3), ")")
  ...
  p <- cowplot::ggdraw() + cowplot::draw_plot(p1,
    ...
    cowplot::draw_plot(p2, x = 0.5, y = 0, width = 0.5,
      height = 0.92) +
  	 cowplot::draw_plot_label(label = "Strongest interaction")
...
plot_response <- function(x, y, x_seq, pred, ci_up,
  ...
  p <- ggplot2::ggplot() +
  	 ggplot2::geom_polygon(data = NULL,
      ggplot2::aes_(x = poly_x, y = poly_y), fill = "dodgerblue3",
      alpha = 0.2) +
  	 ggplot2::geom_line(data = NULL,
      ggplot2::aes_(x = x_seq, y = pred), colour = "dodgerblue3",
      size = 1) +
  	 ggplot2::geom_point(data = NULL,
      ggplot2::aes_(x = x, y = y), shape = 16, size = 1) +
...
plot_predict <- function(x, y_obs, y_pred, ci_up, ci_low,
  ...
  p <- ggplot2::ggplot() +
      ...
      colour = "darkseagreen4", size = 1) +
  	 ggplot2::geom_point(data = NULL,
      ggplot2::aes_(x = x[x_train_zoom], y = y_obs[x_train_zoom]),
      shape = 16, size = 2) +
  	 ggplot2::geom_point(data = NULL,
      ggplot2::aes_(x = x[x_test], y = y_obs[x_test]),
      shape = 17, size = 3) +
  	 ggplot2::geom_point(data = NULL,
...
plot_deriv <- function(press_seq, deriv1, deriv1_ci_low,
  deriv1_ci_up, zic_start_end, zero_in_conf, xlab, ylab,
  pos_text, label) {
	 p <- ggplot2::ggplot() +
	 	 ggplot2::geom_line(data = NULL,
      ggplot2::aes_(x = press_seq, y = deriv1), lty = 1,
      color = "red") +
	 	 ggplot2::geom_line(data = NULL,
      ggplot2::aes_(x = press_seq, y = deriv1_ci_up),
      lty = 2, color = "red") +
	 	 ggplot2::geom_line(data = NULL,
      ggplot2::aes_(x = press_seq, y = deriv1_ci_low),
      lty = 2, color = "red") +
	 	 ggplot2::geom_hline(yintercept = 0,
      linetype = "dashed", color = "black") +
	 	 ggplot2::labs(x = xlab,
      y = paste0("S' (", ylab, ")")) +
	 	 ggplot2::geom_point(data = NULL, ggplot2::aes_(x = press_seq,
      y = deriv1, col = as.factor(zero_in_conf),
      shape = as.factor(zic_start_end))) +
	 	 ggplot2::annotate(geom = "text", x = pos_text$x,
      y = pos_text$y, label = label, hjust = 0) +
    ggplot2::scale_colour_manual(values = c(2,1)) +
    ggplot2::theme(legend.position = "none") +
    ggplot2::ggtitle("1st Derivative S'") +
	 	 plot_outline()
...
plot_all_mod <- function(p1, p2, p3, p4, title) {
	 cowplot::ggdraw() +
		cowplot::draw_plot(p1, x = 0, y = 0.5, width = 0.5,
		  height = 0.45) +
		cowplot::draw_plot(p2, x = 0.5, y = 0.5, width = 0.5,
			 height = 0.45) +
		cowplot::draw_plot(p3, x = 0, y = 0, width = 0.5,
				height = 0.45) +
		cowplot::draw_plot(p4, x = 0.5, y = 0, width = 0.5,
			 height = 0.45) +
